...
async def logo(bot, message):
  try:
    text = message.text.replace("logo","").replace("/","").replace("@OutruiXLogoMakerBot","").strip().upper()
    ...
    await message.reply_photo(logo,caption="**🖼 Logo Generated By @OutruiXLogoMakerBot**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
...
async def logo(bot, message):
  try:
    text = message.text.replace("logosq","").replace("/","").replace("@OutruiXLogoMakerBot","").strip().upper()
